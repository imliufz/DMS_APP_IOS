 ...
 * @type _L4.$|Function
 */
var isAndroidPullrefresh = true;//Android 是否允许使用div 动画
			...
			if (hasPulldown || hasPullup) {
				...
				if (container) {
					...
					if ($container.length === 1) {
						if ($.os.plus && $.os.android && !isAndroidPullrefresh) { //android 5+
							$.plusReady(function() {
 ...
 */
(function($, document) {
	if(!($.os.plus && $.os.android) || isAndroidPullrefresh) { //仅在android的5+版本使用
		return;
	}
